    ...
    def exportSiteDetailsPdf(self, siteList, targetFileOrFolder, timeStamp, isTemp=False):
        ...
        try:
            while query.next():
                ...
                sitePdfs[siteDict['fundortnummer']] = targetFile

                #Clean UP
                #siteDict.clear()
                QgsMapLayerRegistry.instance().removeMapLayers([siteLayer.id(), oekLayer28.id(), oekLayer31.id(), oekLayer34.id()])


        except Exception as e:
            QMessageBox.information(None, "Error", "Error Site Printing: {0}".format(e))
...
class ApisFindSpotPrinter(ApisPrinter):
    ...
    def exportDetailsPdf(self, findSpotList, targetFileOrFolder, timeStamp, isTemp=False):
        ...
        while query.next():
            ...
            # Create PDF
            if len(findSpotList) > 1 or isTemp:
