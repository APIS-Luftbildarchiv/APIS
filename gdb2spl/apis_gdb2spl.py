...
#f=open('sql_dump.sql','w')
#f=codecs.open("sql_dump_2.sql", "w", "utf-8")
...
class ApisSpatialite:
    ...
    def _InitSpatialiteDatabase(self, spatialitePath):
        ...
        try:
            ...
            #print >> f, u"{0};".format(sql)
            #print("{0};".format(sql), file=f)
    ...
    def _CreateSpatialiteTable(self):
        try:
            ...
            for field in self.table['fields']:
                ...
                # if field['targetname'] in self.table['primary']:
    ...
    def _FillSpatialiteTableFromData(self):
        try:
            ...
            #sql = "INSERT INTO {0} VALUES (NULL, {1})".format(self.table['targetname'], ', '.join(map(unicode, ['?' for i in range(len(self.table['fields']))])))
            #print sql
            rows = []
    ...
    def _FillSpatialiteTableFromEsriGdb(self):
        try:
            ...
            #             sqlStr = sqlStr.replace("#?*", unicode(r), 1)
            #         elif r is None:
            #             sqlStr = sqlStr.replace("#?*", "NULL", 1)
            #         else:
            #             sqlStr = sqlStr.replace("#?*", u"'{0}'".format(r), 1)
            #     sqlStr += ";"
            #     print >> f, sqlStr
            #     #print >> f, len(row)
...
if __name__ == '__main__':
    d = datetime.today()
    targetDB = "spatialite/APIS_{0}.sqlite".format(d.strftime("%Y%m%d_%H%M%S"))
    logging.basicConfig(filename='log/{0}.log'.format(d.strftime("%Y%m%d_%H%M%S")),level=logging.DEBUG)
    #logging.basicConfig(filename='{0}.sql'.format(d.strftime("%Y%m%d_%H%M%S")),level=logging.DEBUG)
